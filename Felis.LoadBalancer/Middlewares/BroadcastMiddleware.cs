...
{
	internal class BroadcastMiddleware
	{
		private readonly RequestDelegate _next;
		private readonly LoadBalancingService _loadBalancingService;
		private readonly ILogger<BroadcastMiddleware> _logger;
		private readonly HttpClient _httpClient;

		public BroadcastMiddleware(RequestDelegate next, LoadBalancingService loadBalancingService,
			ILogger<BroadcastMiddleware> logger, HttpClient httpClient)
		{
		...
		public async Task Invoke(HttpContext context)
		{
			...
			try
			{
				...
				var servers = _loadBalancingService.CurrentRouters;
				...
				_logger.LogInformation($"Broadcasting {url} {method} to {string.Join(",", servers)}");
